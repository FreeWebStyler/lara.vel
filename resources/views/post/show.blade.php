...
@section('content')
    ...
    <article class=post postid={{$post->id}}>
        ...
        @if(isset($cans['deleteAll-posts'])) <button type="button" postid="{{$post->id}}" class="btn btn-danger btn-xs" onclick="confirmDelete(this,{{$post->id}})"        >Delete</button>   <!-- [ <a postid={{$post->id}} class=postdel onclick=confirmDelete(this,{{ $post->id }})
        >Delete</a> ] --> @endif
        <div class="content">
...
<script>
    function rate(el, id, sign){ //cl(id + ' ' + sign);
        if((sign == '+' && el.className == 'lime') || (sign == '-' && el.className == 'red')) return;
        $.ajax({
            type: "POST",
            url: "{{ route('comment.rate') }}",
            data: { "_token": $("input[name='_token']").val(), "id": id, "sign" : sign },
            success: function(data) {
                //cl(typeof data);
                if(data == 'true') location.reload(); else {
                    $('#res').html(data);
                    cl(data);
                }
            },
            error: function(jqXHR, textStatus, errorThrown) { // vvv---- This is the new bit
